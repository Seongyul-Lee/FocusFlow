...
## Snapshot
Revision: 4
Last updated (KST): 2025-12-21 21:00

## Current
- State: Traceability(Preview baseline + immutable deploy) 고정 완료. P0 잔여는 아직 오픈. (Repo: Seongyul-Lee/pomobox / Prod: https://pomobox.app)
...
## Now (exactly 1)
- P0 잔여 해결 준비: running 중 duration 변경 시 진행/통계 불일치 + Reset/Skip 정합(정의-구현-UI) 이슈를 Patch Room(#3) 티켓으로 분리/위임

## Blockers/Risks (P0 only)
- P0: running 중 duration 변경 시 진행/통계 불일치, Reset/Skip 정합(정의-구현-UI) — evidence/owner/ticket TBD
...
- [2025-12-19] Decision: Background drift 시나리오 PASS를 immutable Preview deploy로 고정.
